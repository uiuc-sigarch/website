...
1. *What is computer architecture?*
   ...
   designed, built, and organized.
   It defines the operation of a computer, and provides the interface for interaction with the system and other devices.

2. *Why is it important?*

   Computers must constantly get faster to keep up with increasingly demanding workloads. While speedup is achievable at the software level via various optimization techniques, any such improvement will be too specific to benefit more than a trivial percentage of tasks. Improving the performance of the CPU itself, however, can provide a universal speedup that benefits all (or at least most) applications.
   In past years, we have relied on Moore's law for this improvement, since the growing number of transistors on a single chip means more computing power can fit into a single device. Unfortunately, we are fast approaching the theoretical limit of transistor size and can no longer rely on simply adding compute power. Instead, we must rethink the design of computers  to better utilize the resources we have available.
